...
- Handlers: There are two default handlers will be added to both CLI parent logger and root logger. One is a colorized
            ...
            disabled through 'mg configure' command. The logging file locates at path defined in MG_LOGFILE_DIR.
...
from azure.core.pipeline.policies._universal import HttpLoggingPolicy
from msgraph.cli.core.commands.events import EVENT_INVOKER_PRE_CMD_TBL_TRUNCATE
...
class MgCliLogging(CLILogging):
    ...
    def __init__(self, name, cli_ctx=None):
        ...
        self.cli_ctx.register_event(EVENT_INVOKER_PRE_CMD_TBL_TRUNCATE,
                                    MgCliLogging.init_command_file_logging)
        self.cli_ctx.register_event(EVENT_CLI_POST_EXECUTE,
    ...
    def configure(self, args):
        ...
        if self.log_level == CliLogLevel.DEBUG:
            ...
            logging.getLogger("msgraph.cli.core.mglogging.custom_http_logging_policy").setLevel(
                logging.DEBUG)
    ...
    def init_command_file_logging(cli_ctx, **kwargs):
        ...
        if not cli_ctx.data.get('completer_active', False):
            ...
            cmd_logger = logging.getLogger(MgCliLogging._COMMAND_METADATA_LOGGER)
    ...
    def _init_command_logfile_handlers(self, command_metadata_logger, args):
        ...
        args = MgCliLogging._get_clean_args(command if command != _UNKNOWN_COMMAND else None, args)
        ...
        for i, arg in enumerate(args):
            ...
            if arg.startswith('-') and not arg.startswith('---') and len(arg) > 1:
                ...
                if not arg.startswith("--"):
                    ...
                    opt = opt + "=" if len(
                        arg) > 2 and arg[2] == "=" else opt  # append '=' if necessary
    ...
    def deinit_cmd_metadata_logging(cli_ctx):
        cli_ctx.logging.end_cmd_metadata_logging(
            cli_ctx.result.exit_code if cli_ctx.result else 128)
...
class CommandLoggerContext:
    def __init__(self, module_logger):
        ...
        self.hdlr = logging.getLogger(MgCliLogging._COMMAND_METADATA_LOGGER)  # pylint: disable=protected-access
