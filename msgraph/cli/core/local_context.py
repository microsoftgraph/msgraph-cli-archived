...
class AzCLILocalContext:  # pylint: disable=too-many-instance-attributes
    ...
    def initialize(self):
        ...
        try:
            ...
            if self.is_on:
                logger.warning(
                    'The working directory has been deleted or recreated. Parameter persistence is ignored.'
                )
    ...
    def _load_local_context_files(self, recursive=False):
        ...
        if self.username and self.current_dir:
            ...
            while current_dir:
                ...
                if os.path.isfile(file_path) and os.access(file_path, os.R_OK) and os.access(
                        file_path, os.W_OK):
                    local_context_files.append(
                        _ConfigFile(dir_path, file_path, LOCAL_CONTEXT_NOTICE))
                    if not recursive:
    ...
    def effective_working_directory(self):
        return os.path.dirname(
            self._local_context_file.config_dir) if self._local_context_file else ''
    ...
    def set(self, scopes, argument, value):
        if self.is_on and self.username and self.current_dir:
            if self._local_context_file is None:
                file_path = os.path.join(self.current_dir, self.dir_name,
                                         self._get_local_context_file_name())
    ...
    def turn_on(self):
        ...
        self.is_on = self.config.getboolean(LOCAL_CONTEXT_ON_OFF_CONFIG_SECTION, self.username,
                                            False)
        self._local_context_file = self._get_local_context_file()
    ...
    def delete(self, names=None):
        ...
        logger.warning(
            'Parameter persistence value is deleted. You can run `az config param-persist show` to show all '
            'available values.')
