...
try:
    ...
    try:
        ...
        logger.info("Command ran in %.3f seconds (init: %.3f, invoke: %.3f)",
                    invoke_finish_time - start_time, init_finish_time - start_time,
    ...
    try:
        ...
        if exit_code == 0 and az_cli.config.getboolean('auto-upgrade', 'enable', False) and \
            ...
            if VERSIONS[_VERSION_UPDATE_TIME]:
                import datetime
                version_update_time = datetime.datetime.strptime(VERSIONS[_VERSION_UPDATE_TIME],
                                                                 '%Y-%m-%d %H:%M:%S.%f')
                ...
                if LooseVersion(VERSIONS['versions']['core']['local']) < LooseVersion(VERSIONS['versions']['core']['pypi']):  # pylint: disable=line-too-long
                    ...
                    if prompt:
                        ...
                        cmd.append('-y')
                        upgrade_exit_code = subprocess.call(cmd,
