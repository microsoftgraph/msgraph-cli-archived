...
|**--media-config**|dictionary|mediaConfig|media_config|mediaConfig|
|**--meeting-info**|dictionary|meetingInfo|meeting_info|meetingInfo|
...
|**--recording-info-initiator-user-display-name**|string|The identity's display name. Note that this may not always be available or up to date. For example, if a user changes their display name, the API may show the new value in a future response, but the items associated with the user won't show up as having changed when using delta.|display_name|displayName|
|**--recording-info-initiator-user-id**|string|Unique identifier for the identity.|microsoft_graph_identity_id|id|
|**--recording-info-initiator-device-display-name**|string|The identity's display name. Note that this may not always be available or up to date. For example, if a user changes their display name, the API may show the new value in a future response, but the items associated with the user won't show up as having changed when using delta.|microsoft_graph_identity_display_name|displayName|
|**--recording-info-initiator-device-id**|string|Unique identifier for the identity.|id1|id|
|**--recording-info-initiator-application-display-name**|string|The identity's display name. Note that this may not always be available or up to date. For example, if a user changes their display name, the API may show the new value in a future response, but the items associated with the user won't show up as having changed when using delta.|display_name1|displayName|
|**--recording-info-initiator-application-id**|string|Unique identifier for the identity.|id2|id|
