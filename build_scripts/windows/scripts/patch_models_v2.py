...
def solve_one_model(models_module, output_folder):
    ...
    if enum_models_classes:
        ...
        shutil.copyfile(enum_file, Path(
            output_folder, as_file_name(enum_file_module_name)))
    else:
        enum_file_module_name = None

    write_model_file(
        Path(output_folder, as_file_name(MODEL_PY3_NAME)), models_classes)
    write_model_file(Path(output_folder, as_file_name(
        MODEL_PY2_NAME)), py2_models_classes)
    write_paging_file(Path(output_folder, as_file_name(
        PAGED_NAME)), paged_models_classes)
    write_complete_init(
...
def write_complete_init(output_file_path, models, exceptions_classes, paging_models, enum_models, enum_file_module_name):
    with open(output_file_path, "bw") as write_fd:
        ...
        for _, model_name, _, _ in models:
            write_fd.write(
                f"    from .{MODEL_PY3_NAME} import {model_name}\n".encode("utf-8"))
        # Py 3 exceptions
        ...
        for _, model_name, _, _ in models:
            write_fd.write(
                f"    from .{MODEL_PY2_NAME} import {model_name}\n".encode("utf-8"))
        # Py 2 exceptions
        for model_name in exceptions_classes:
            write_fd.write(
                f"    from .{MODEL_PY2_NAME} import {model_name}\n".encode("utf-8"))
        ...
        for model_name, _, _ in paging_models:
            write_fd.write(
                f"from .{PAGED_NAME} import {model_name}\n".encode("utf-8"))

        if enum_models:
            # Write enum model import
            for model_name, _, _ in enum_models:
                write_fd.write(
                    f"from .{enum_file_module_name} import {model_name}\n".encode("utf-8"))
